  ...
  return await new Promise((resolve, reject) => {
    ...
    // sharedStrings.xml
    {
      ...
      if (
        ...
        strSharedStrings +=
          `<sst xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" count="${intBubbleCols}" uniqueCount="${intBubbleCols}">`;
        ...
        strSharedStrings +=
          `<sst xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" count="${totCount}" uniqueCount="${totCount}">`;
      ...
      if (
        ...
        data.forEach((objData, idx) => {
          if (idx === 0) strSharedStrings += "<si><t>X-Axis</t></si>";
            ...
            strSharedStrings += `<si><t>${
              encodeXmlEntities(objData.name || `Y-Axis${idx}`)
            }</t></si>`;
            strSharedStrings += `<si><t>${
    ...
    // worksheets/sheet1.xml
    {
      ...
      if (
        ...
        for (let idx = 1; idx < intBubbleCols; idx++) {
          ...
          }1" t="s"><v>${idx}</v></c>`; // NOTE: add `t="s"` for label cols!
        }
        strSheetXml += "</row>";
        ...
        if (!IS_MULTI_CAT_AXES) {
          ...
          for (let idx = 0; idx < TOT_CAT; idx++) {
            ...
            revLabelGroups.forEach((labelsGroup, idy) => {
              ...
              if (colLabel) {
                const totGrpLbls = idy === 0
                  ? 1
                  : revLabelGroups[idy - 1].filter((label) =>
                    label && label !== ""
                  ).length; // get unique label so we can add to get proper shared-string #
                totLabels += totGrpLbls;
                strSheetXml += `<c r="${getExcelColName(idx + 1 + idy)}${
                  idx + 2
                }" t="s"><v>${totLabels}</v></c>`;
              }
            });
...
export function makeXmlCharts(rel: ISlideRelChart): string {
  ...
  // STEP 1: Create chart
  {
    ...
    if (rel.opts._type === CHART_TYPE.BAR3D) {
      strXml +=
        ...
        }"/><c:perspective val="${rel.opts.v3DPerspective}"/></c:view3D>`;
    }
  ...
  if (
    ...
    // Param check
    if (
      rel.opts.valAxes && rel.opts.valAxes.length > 1 && !usesSecondaryValAxis
    ) {
      throw new Error(
    ...
    if (rel.opts.catAxes) {
      if (
        !rel.opts.valAxes || rel.opts.valAxes.length !== rel.opts.catAxes.length
      ) {
        throw new Error(
  ...
  // C: Chart Properties and plotArea Options: Border, Data Table, Fill, Legend
  {
    ...
    if (rel.opts.showDataTable) {
      strXml += "<c:dTable>";
      strXml += `  <c:showHorzBorder val="${
        !rel.opts.showDataTableHorzBorder ? 0 : 1
      }"/>`;
      strXml += `  <c:showVertBorder val="${
        !rel.opts.showDataTableVertBorder ? 0 : 1
      }"/>`;
      strXml += `  <c:showOutline    val="${
        !rel.opts.showDataTableOutline ? 0 : 1
      }"/>`;
      strXml += `  <c:showKeys       val="${
        !rel.opts.showDataTableKeys ? 0 : 1
      }"/>`;
      strXml += "  <c:spPr>";
      strXml += "    <a:noFill/>";
      strXml +=
      ...
      strXml += '     <a:pPr rtl="0">';
      strXml += `       <a:defRPr sz="${
        Math.round((rel.opts.dataTableFontSize || DEF_FONT_SIZE) * 100)
    ...
    if (rel.opts.showLegend) {
      ...
      if (
        rel.opts.legendFontFace || rel.opts.legendFontSize ||
        ...
        strXml += rel.opts.legendFontSize
          ? `<a:defRPr sz="${
            Math.round(Number(rel.opts.legendFontSize) * 100)
          }">`
          : "<a:defRPr>";
  ...
  switch (chartType) {
    ...
    case CHART_TYPE.RADAR:
      ...
      data.forEach((obj) => {
        ...
        strXml += "<c:ser>";
        strXml +=
          `  <c:idx val="${obj._dataIndex}"/><c:order val="${obj._dataIndex}"/>`;
        strXml += "  <c:tx>";
        strXml += "    <c:strRef>";
        strXml += "      <c:f>Sheet1!$" +
          getExcelColName(obj._dataIndex + obj.labels.length + 1) + "$1</c:f>";
        strXml += '      <c:strCache><c:ptCount val="1"/><c:pt idx="0"><c:v>' +
          encodeXmlEntities(obj.name) + "</c:v></c:pt></c:strCache>";
        strXml += "    </c:strRef>";
        strXml += "  </c:tx>";
        ...
        if (seriesColor === "transparent") {
          ...
          strXml += "<a:solidFill>" + createColorElement(seriesColor) +
            "</a:solidFill>";
        }
        ...
        if (chartType !== CHART_TYPE.RADAR) {
          ...
          strXml += `<c:showCatName val="0"/><c:showSerName val="${
            opts.showSerName ? "1" : "0"
          }"/><c:showPercent val="0"/><c:showBubbleSize val="0"/>`;
          strXml += `<c:showLeaderLines val="${
        ...
        // 2: "Categories"
        {
          ...
          if (opts.catLabelFormatCode) {
            ...
            strXml += "    <c:numCache>";
            strXml += "      <c:formatCode>" +
              (opts.catLabelFormatCode || "General") + "</c:formatCode>";
            ...
            strXml += `    <c:f>Sheet1!$A$2:$${
              getExcelColName(obj.labels.length)
            }$${obj.labels[0].length + 1}</c:f>`;
        ...
        // 3: "Values"
        {
          ...
          }$2:$${getExcelColName(obj._dataIndex + obj.labels.length + 1)}$${
            obj.labels[0].length + 1
          }</c:f>`;
          strXml += "    <c:numCache>";
          strXml += "      <c:formatCode>" +
      ...
      strXml +=
        `<c:axId val="${catAxisId}"/><c:axId val="${valAxisId}"/><c:axId val="${AXIS_ID_SERIES_PRIMARY}"/>`;
    ...
    case CHART_TYPE.SCATTER:
      ...
      data.filter((_obj, idx) => idx > 0).forEach((series, idx) => {
        ...
        strXml += "  <c:spPr>";
        {
          ...
          if (tmpSerColor === "transparent") {
            ...
            strXml += "<a:solidFill>" + createColorElement(tmpSerColor) +
              "</a:solidFill>";
          }
          ...
          if (
            ...
            obj.labels[0].forEach((label, idx) => {
              if (
                ...
                strXml += "              <a:r>";
                strXml += '                    <a:rPr lang="' +
                  (opts.lang || "en-US") + '" dirty="0"/>';
                strXml += "                    <a:t>" +
                  encodeXmlEntities(label) + "</a:t>";
                strXml += "              </a:r>";
                ...
                if (
                  ...
                  strXml += "              <a:r>";
                  strXml += '                  <a:rPr lang="' +
                    (opts.lang || "en-US") + '" baseline="0" dirty="0"/>';
                  strXml += "                  <a:t> (</a:t>";
                  strXml += "              </a:r>";
                  strXml += '              <a:fld id="{' +
                    ...
                    '}" type="XVALUE">';
                  strXml += '                  <a:rPr lang="' +
                    (opts.lang || "en-US") + '" baseline="0"/>';
                  strXml += "                  <a:pPr>";
                  strXml += "                      <a:defRPr/>";
                  strXml += "                  </a:pPr>";
                  strXml += "                  <a:t>[" +
                    encodeXmlEntities(obj.name) + "</a:t>";
                  strXml += "              </a:fld>";
                  strXml += "              <a:r>";
                  strXml += '                  <a:rPr lang="' +
                    (opts.lang || "en-US") + '" baseline="0" dirty="0"/>';
                ...
                strXml += `            <c16:uniqueId val="{${
                  "00000000".substring(0, 8 - (idx + 1).toString().length)
                    .toString()
                }${idx + 1}${chartUuid}}"/>`;
                strXml += "      </c:ext>";
                strXml += "        </c:extLst>";
      ...
      // 3: Data Labels
      {
        ...
        }" i="${opts.dataLabelFontItalic ? "1" : "0"}" strike="noStrike" sz="${
          Math.round((opts.dataLabelFontSize || DEF_FONT_SIZE) * 100)
        }" u="none">`;
        strXml += "          <a:solidFill>" +
          createColorElement(opts.dataLabelColor || DEF_FONT_COLOR) +
    ...
    case CHART_TYPE.BUBBLE3D:
      ...
      data.filter((_obj, idx) => idx > 0).forEach((series, idx) => {
        ...
        strXml += "  <c:tx>";
        strXml += "    <c:strRef>";
        strXml += "      <c:f>Sheet1!$" + getExcelColName(idxColLtr + 1) +
          "$1</c:f>";
        strXml += '      <c:strCache><c:ptCount val="1"/><c:pt idx="0"><c:v>' +
          encodeXmlEntities(obj.name) + "</c:v></c:pt></c:strCache>";
        strXml += "    </c:strRef>";
      ...
      // 3: Data Labels
      {
        ...
        strXml +=
          '  <c:ext uri="{CE6537A1-D6FC-4f65-9D91-7224C49458BB}" xmlns:c15="http://schemas.microsoft.com/office/drawing/2012/chart">';
        strXml += '    <c15:showLeaderLines val="' +
          (opts.showLeaderLines ? "1" : "0") + '"/>';
        strXml += "  </c:ext>";
        strXml += "</c:extLst>";
        strXml += "</c:dLbls>";
    ...
    case CHART_TYPE.PIE:
      ...
      strXml += '        <c:pt idx="0"><c:v>' +
        encodeXmlEntities(optsChartData.name) + "</c:v></c:pt>";
      strXml += "      </c:strCache>";
      strXml += "    </c:strRef>";
      strXml += "  </c:tx>";
      strXml += "  <c:spPr>";
      ...
      optsChartData.labels[0].forEach((_label, idx) => {
        ...
        strXml += `   <a:defRPr sz="${
          Math.round((opts.dataLabelFontSize || DEF_FONT_SIZE) * 100)
        }" b="${opts.dataLabelFontBold ? 1 : 0}" i="${
          opts.dataLabelFontItalic ? 1 : 0
      ...
      strXml += "  <c:strRef>";
      strXml += `    <c:f>Sheet1!$A$2:$A$${
        optsChartData.labels[0].length + 1
      }</c:f>`;
      strXml += "    <c:strCache>";
      ...
      strXml += "  <c:val>";
      strXml += "    <c:numRef>";
      strXml += `      <c:f>Sheet1!$B$2:$B$${
        optsChartData.labels[0].length + 1
      }</c:f>`;
      strXml += "      <c:numCache>";
      strXml += `           <c:ptCount val="${
        optsChartData.labels[0].length
      }"/>`;
      ...
      // 4: Close "SERIES"
      strXml += "  </c:ser>";
      strXml += `  <c:firstSliceAng val="${
        opts.firstSliceAng ? Math.round(opts.firstSliceAng) : 0
      }"/>`;
      if (chartType === CHART_TYPE.DOUGHNUT) {
        strXml += `<c:holeSize val="${
          typeof opts.holeSize === "number" ? opts.holeSize : "50"
        }"/>`;
      }
      strXml += "</c:" + chartType + "Chart>";
...
function makeCatAxis(
  ...
  if (
    opts._type === CHART_TYPE.SCATTER || opts._type === CHART_TYPE.BUBBLE ||
  ...
  if (
    ...
    strXml += '  <c:numFmt formatCode="' + (opts.valAxisLabelFormatCode
  ...
  if (opts._type === CHART_TYPE.SCATTER) {
    ...
    strXml += '  <c:majorTickMark val="' +
      (opts.catAxisMajorTickMark || "out") + '"/>';
    strXml += '  <c:minorTickMark val="' +
  ...
  if (opts.catAxisLabelRotate) {
    strXml += `<a:bodyPr rot="${
      convertRotationDegrees(opts.catAxisLabelRotate)
    }"/>`;
  } else {
  ...
  }" b="${opts.catAxisLabelFontBold ? 1 : 0}" i="${
  ...
  } val="${opts.valAxisCrossesAt || "autoZero"}"/>`;
  strXml += ' <c:auto val="1"/>';
  ...
  if (
    opts.catLabelFormatCode || opts._type === CHART_TYPE.SCATTER ||
...
function makeValAxis(opts: IChartOptsLib, valAxisId: string): string {
  ...
  const crossAxId = valAxisId === AXIS_ID_VALUE_PRIMARY
    ? AXIS_ID_CATEGORY_PRIMARY
    : AXIS_ID_CATEGORY_SECONDARY;
  let strXml = "";
  ...
  strXml += `  <a:bodyPr${
    opts.valAxisLabelRotate
      ? (' rot="' + convertRotationDegrees(opts.valAxisLabelRotate).toString() +
        '"')
      : ""
  ...
  }" b="${opts.valAxisLabelFontBold ? 1 : 0}" i="${
  ...
  strXml += ' <c:crossBetween val="' +
    (opts._type === CHART_TYPE.SCATTER ||
        (!!(Array.isArray(opts._type) &&
          opts._type.filter((type) => type.type === CHART_TYPE.AREA).length >
            0))
      ? "midCat"
      : "between") +
    '"/>';
  ...
  if (opts.valAxisDisplayUnit) {
    strXml += `<c:dispUnits><c:builtInUnit val="${opts.valAxisDisplayUnit}"/>${
      opts.valAxisDisplayUnitLabel ? "<c:dispUnitsLbl/>" : ""
    }</c:dispUnits>`;
  }
...
function makeSerAxis(
  ...
  strXml += "<c:serAx>";
  strXml += '  <c:axId val="' + axisId + '"/>';
  strXml += '  <c:scaling><c:orientation val="' +
  ...
  strXml += "    <a:pPr>";
  strXml += `    <a:defRPr sz="${
    Math.round((opts.serAxisLabelFontSize || DEF_FONT_SIZE) * 100)
  }" b="${opts.serAxisLabelFontBold ? "1" : "0"}" i="${
    opts.serAxisLabelFontItalic ? "1" : "0"
  }" u="none" strike="noStrike">`;
  strXml += `      <a:solidFill>${
    createColorElement(opts.serAxisLabelColor || DEF_FONT_COLOR)
  }</a:solidFill>`;
  strXml += `      <a:latin typeface="${
  ...
  if (opts.serLabelFormatCode) {
    ...
    if (opts.serAxisMajorTimeUnit) {
      strXml +=
        ` <c:majorTimeUnit val="${opts.serAxisMajorTimeUnit.toLowerCase()}"/>`;
    }
    if (opts.serAxisMinorTimeUnit) {
      strXml +=
        ` <c:minorTimeUnit val="${opts.serAxisMinorTimeUnit.toLowerCase()}"/>`;
    }
...
function genXmlTitle(
  ...
  let layout = "<c:layout/>";
  if (
    opts.titlePos && typeof opts.titlePos.x === "number" &&
    ...
    layout =
      `<c:layout><c:manualLayout><c:xMode val="edge"/><c:yMode val="edge"/><c:x val="${valX}"/><c:y val="${valY}"/></c:manualLayout></c:layout>`;
  }
        ...
        <c:rich>
          ...
          <a:p>
            ...
            <a:defRPr ${sizeAttr} b="${titleBold}" i="0" u="none" strike="noStrike">
              <a:solidFill>${
    createColorElement(opts.color || DEF_FONT_COLOR)
  }</a:solidFill>
...
function getExcelColName(colIndex: number): string {
  ...
  if (colIdx <= 25) {
    ...
    // AA-ZZ (ZZ = index 702)
    colStr = `${LETTERS[Math.floor(colIdx / LETTERS.length - 1)]}${
      LETTERS[colIdx % LETTERS.length]
    }`;
  }
...
function createShadowElement(
  ...
  strXml +=
    `<a:${type}Shdw sx="100000" sy="100000" kx="0" ky="0"  algn="bl" blurRad="${blur}" rotWithShape="${rotShape}" dist="${offset}" dir="${angle}">`;
