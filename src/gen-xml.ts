...
function slideObjectToXml(slide: PresSlide | SlideLayout): string {
  ...
  if (slide._bkgdImgRid) {
    strSlideXml +=
      `<p:bg><p:bgPr><a:blipFill dpi="0" rotWithShape="1"><a:blip r:embed="rId${slide._bkgdImgRid}"><a:lum/></a:blip><a:srcRect/><a:stretch><a:fillRect/></a:stretch></a:blipFill><a:effectLst/></p:bgPr></p:bg>`;
  } else if (slide.background?.color) {
    strSlideXml += `<p:bg><p:bgPr>${
      genXmlColorSelection(slide.background)
    }</p:bgPr></p:bg>`;
  } else if (
    !slide.bkgd && slide._name && slide._name === DEF_PRES_LAYOUT_NAME
  ) {
  ...
  slide._slideObjects.forEach((slideItemObj: ISlideObject, idx: number) => {
    ...
    switch (slideItemObj._type) {
      case SLIDE_OBJECT_TYPES.table:
        ...
        if (Array.isArray(objTabOpts.colW)) {
          ...
          for (let col = 0; col < intColCnt; col++) {
            ...
            if (w == null || isNaN(w)) {
              w = (typeof slideItemObj.options.w === "number"
          ...
          cells.forEach((cellObj) => {
            ...
            const cellSpanAttrs = {
              rowSpan: cell.options?.rowspan > 1
                ...
                : undefined,
              gridSpan: cell.options?.colspan > 1
            ...
            const cellValign = cellOpts.valign
              ? ` anchor="${
                cellOpts.valign.replace(/^c$/i, "ctr").replace(/^m$/i, "ctr")
                  .replace("center", "ctr").replace("middle", "ctr").replace(
                    "top",
                    "t",
                  ).replace("btm", "b").replace("bottom", "b")
              }"`
              : "";
            const cellTextDir =
            ...
            const fillColor = cellOpts.fill || inheritedFill;
            const cellFill = fillColor ? genXmlColorSelection(fillColor) : "";

            let cellMargin = cellOpts.margin === 0 || cellOpts.margin
              ? cellOpts.margin
              : DEF_CELL_MARGIN_IN;
            ...
            let cellMarginXml = "";
            if (cellMargin[0] >= 1) {
              cellMarginXml = ` marL="${valToPts(cellMargin[3])}" marR="${
                valToPts(cellMargin[1])
              }" marT="${valToPts(cellMargin[0])}" marB="${
                valToPts(
                  cellMargin[2],
                )
              }"`;
            } else {
              cellMarginXml = ` marL="${inch2Emu(cellMargin[3])}" marR="${
                inch2Emu(cellMargin[1])
              }" marT="${inch2Emu(cellMargin[0])}" marB="${
                inch2Emu(
                  cellMargin[2],
                )
              }"`;
            ...
            if (cellOpts.border && Array.isArray(cellOpts.border)) {
              ...
              [
                ...
                if (border.type !== "none") {
                  ...
                  strXml +=
                    `<a:${obj.name} w="0" cap="flat" cmpd="sng" algn="ctr"><a:noFill/></a:${obj.name}>`;
                }
              });
            }
      ...
      case SLIDE_OBJECT_TYPES.placeholder:
        ...
        strSlideXml += `<p:nvSpPr><p:cNvPr id="${
          idx + 2
        }" name="${slideItemObj.options.objectName}">`;
        // <Hyperlink>
        if (slideItemObj.options.hyperlink?.url) {
          strSlideXml +=
            `<a:hlinkClick r:id="rId${slideItemObj.options.hyperlink._rId}" tooltip="${
        ...
        if (slideItemObj.shape === "custGeom") {
          ...
          strSlideXml += '<a:prstGeom prst="' + slideItemObj.shape +
            '"><a:avLst>';
          if (slideItemObj.options.rectRadius) {
            strSlideXml += `<a:gd name="adj" fmla="val ${
              Math.round(
            ...
            for (let i = 0; i < 2; i++) {
              ...
              strSlideXml += `<a:gd name="adj${i + 1}" fmla="val ${
                convertRotationDegrees(angle)
              }" />`;
            }

            if (slideItemObj.options.arcThicknessRatio) {
              strSlideXml += `<a:gd name="adj3" fmla="val ${
                Math.round(slideItemObj.options.arcThicknessRatio * 50000)
              }" />`;
            }
          }
        ...
        if (slideItemObj.options.line) {
          ...
          if (slideItemObj.options.line.dashType) {
            strSlideXml +=
              `<a:prstDash val="${slideItemObj.options.line.dashType}"/>`;
          }
          if (slideItemObj.options.line.beginArrowType) {
            strSlideXml +=
              `<a:headEnd type="${slideItemObj.options.line.beginArrowType}"/>`;
          }
          if (slideItemObj.options.line.endArrowType) {
            strSlideXml +=
              `<a:tailEnd type="${slideItemObj.options.line.endArrowType}"/>`;
          }
        ...
        if (
          ...
          slideItemObj.options.shadow.type = slideItemObj.options.shadow.type ||
            "outer";
          ...
          strSlideXml += ` <a:${slideItemObj.options.shadow.type}Shdw ${
            slideItemObj.options.shadow.type === "outer"
              ...
              : ""
          } blurRad="${slideItemObj.options.shadow.blur}" dist="${slideItemObj.options.shadow.offset}" dir="${slideItemObj.options.shadow.angle}">`;
          strSlideXml +=
            ` <a:srgbClr val="${slideItemObj.options.shadow.color}">`;
          strSlideXml +=
            ` <a:alpha val="${slideItemObj.options.shadow.opacity}"/></a:srgbClr>`;
      ...
      case SLIDE_OBJECT_TYPES.image:
        ...
        strSlideXml += `<p:cNvPr id="${
          idx + 2
        }" name="${slideItemObj.options.objectName}" descr="${
          encodeXmlEntities(
        ...
        if (slideItemObj.hyperlink?.url) {
          strSlideXml +=
            `<a:hlinkClick r:id="rId${slideItemObj.hyperlink._rId}" tooltip="${
        ...
        if (
          (slide._relsMedia || []).filter((rel) =>
            rel.rId === slideItemObj.imageRid
          )[0] &&
          (slide._relsMedia || []).filter((rel) =>
              rel.rId === slideItemObj.imageRid
            )[0].extn === "svg"
          ...
          strSlideXml += slideItemObj.options.transparency
            ? ` <a:alphaModFix amt="${
              Math.round((100 - slideItemObj.options.transparency) * 1000)
            }"/>`
            : "";
          strSlideXml += " <a:extLst>";
          strSlideXml +=
          ...
          strSlideXml +=
            `   <asvg:svgBlip xmlns:asvg="http://schemas.microsoft.com/office/drawing/2016/SVG/main" r:embed="rId${slideItemObj.imageRid}"/>`;
          strSlideXml += "  </a:ext>";
          strSlideXml += " </a:extLst>";
          strSlideXml += "</a:blip>";
        } else {
      ...
      case SLIDE_OBJECT_TYPES.media:
        if (slideItemObj.mtype === "online") {
          ...
          strSlideXml += " <p:spPr>";
          strSlideXml +=
            `  <a:xfrm${locationAttr}><a:off x="${x}" y="${y}"/><a:ext cx="${cx}" cy="${cy}"/></a:xfrm>`;
          ...
          strSlideXml += `<p:cNvPr id="${
            slideItemObj.mediaRid + 2
          }" name="${slideItemObj.options.objectName}"><a:hlinkClick r:id="" action="ppaction://media"/></p:cNvPr>`;
          strSlideXml +=
            ' <p:cNvPicPr><a:picLocks noChangeAspect="1"/></p:cNvPicPr>';
          strSlideXml += " <p:nvPr>";
          strSlideXml +=
      ...
      case SLIDE_OBJECT_TYPES.chart:
        ...
        strSlideXml += `   <p:cNvPr id="${
          ...
          encodeXmlEntities(slideItemObj.options.altText || "")
        }"/>`;
  ...
  if (slide._slideNumberProps) {
    ...
    if (
      ...
      strSlideXml += ` lIns="${
        valToPts(slide._slideNumberProps.margin[3] || 0)
      }"`;
      strSlideXml += ` tIns="${
        valToPts(slide._slideNumberProps.margin[0] || 0)
      }"`;
      strSlideXml += ` rIns="${
        valToPts(slide._slideNumberProps.margin[1] || 0)
      }"`;
      strSlideXml += ` bIns="${
        valToPts(slide._slideNumberProps.margin[2] || 0)
      }"`;
    ...
    if (slide._slideNumberProps.valign) {
      strSlideXml += ` anchor="${
        slide._slideNumberProps.valign.replace("top", "t").replace(
          "middle",
          "ctr",
        ).replace("bottom", "b")
      }"`;
    ...
    if (
      slide._slideNumberProps.fontFace || slide._slideNumberProps.fontSize ||
      slide._slideNumberProps.color
    ) {
      strSlideXml += `<a:defRPr sz="${
        Math.round((slide._slideNumberProps.fontSize || 12) * 100)
      }">`;
      ...
      if (slide._slideNumberProps.fontFace) {
        strSlideXml +=
          `<a:latin typeface="${slide._slideNumberProps.fontFace}"/><a:ea typeface="${slide._slideNumberProps.fontFace}"/><a:cs typeface="${slide._slideNumberProps.fontFace}"/>`;
      }
    ...
    strSlideXml +=
      `<a:t>${slide._slideNum}</a:t></a:fld><a:endParaRPr lang="en-US"/></a:p>`;
...
function slideObjectRelationsToXml(
  ...
  slide._rels.forEach((rel: ISlideRel) => {
    ...
    if (rel.type.toLowerCase().includes("hyperlink")) {
      if (rel.data === "slide") {
        strXml +=
          `<Relationship Id="rId${rel.rId}" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/slide" Target="slide${rel.Target}.xml"/>`;
      } else {
        strXml +=
  ...
  (slide._relsChart || []).forEach((rel: ISlideRelChart) => {
    lastRid = Math.max(lastRid, rel.rId);
    strXml +=
      `<Relationship Id="rId${rel.rId}" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/chart" Target="${rel.Target}"/>`;
  });
  (slide._relsMedia || []).forEach((rel: ISlideRelMedia) => {
    const relRid = rel.rId.toString();
    lastRid = Math.max(lastRid, rel.rId);
    if (rel.type.toLowerCase().includes("image")) {
      strXml += '<Relationship Id="rId' + relRid +
      ...
      if (strXml.includes(' Target="' + rel.Target + '"')) {
        ...
        strXml += '<Relationship Id="rId' + relRid + '" Target="' + rel.Target +
...
function genXmlParagraphProperties(
  ...
  // A: Build paragraphProperties
  {
    ...
    if (textObj.options.lineSpacing) {
      strXmlLnSpc = `<a:lnSpc><a:spcPts val="${
        Math.round(textObj.options.lineSpacing * 100)
      }"/></a:lnSpc>`;
    } else if (textObj.options.lineSpacingMultiple) {
      strXmlLnSpc = `<a:lnSpc><a:spcPct val="${
        Math.round(textObj.options.lineSpacingMultiple * 100000)
      }"/></a:lnSpc>`;
    }
    ...
    if (
      ...
      strXmlParaSpc += `<a:spcBef><a:spcPts val="${
        Math.round(textObj.options.paraSpaceBefore * 100)
      }"/></a:spcBef>`;
    }
    if (
      ...
      strXmlParaSpc += `<a:spcAft><a:spcPts val="${
        Math.round(textObj.options.paraSpaceAfter * 100)
      }"/></a:spcAft>`;
    }
    ...
    if (typeof textObj.options.bullet === "object") {
      ...
      if (textObj.options.bullet.type) {
        ...
        strXmlBullet =
          `<a:buSzPct val="100000"/><a:buChar char="${BULLET_TYPES.DEFAULT}"/>`;
      }
    ...
    if (textObj.options.tabStops && Array.isArray(textObj.options.tabStops)) {
      const tabStopsXml = textObj.options.tabStops.map((stop) =>
...
function genXmlTextRunProperties(
  ...
  if (
    opts.color || opts.fontFace || opts.outline ||
    (typeof opts.underline === "object" && opts.underline.color)
  ) {
    if (opts.outline && typeof opts.outline === "object") {
      runProps += `<a:ln w="${valToPts(opts.outline.size || 0.75)}">${
    ...
    if (opts.highlight) {
      runProps += `<a:highlight>${
        createColorElement(opts.highlight)
      }</a:highlight>`;
    }
    if (typeof opts.underline === "object" && opts.underline.color) {
      runProps += `<a:uFill>${
        genXmlColorSelection(opts.underline.color)
      }</a:uFill>`;
    }
    if (opts.glow) {
      runProps += `<a:effectLst>${
        createGlowElement(opts.glow, DEF_TEXT_GLOW)
      }</a:effectLst>`;
    }
    if (opts.fontFace) {
      // NOTE: 'cs' = Complex Script, 'ea' = East Asian (use "-120" instead of "0" - per Issue #174); ea must come first (Issue #174)
      runProps +=
        `<a:latin typeface="${opts.fontFace}" pitchFamily="34" charset="0"/><a:ea typeface="${opts.fontFace}" pitchFamily="34" charset="-122"/><a:cs typeface="${opts.fontFace}" pitchFamily="34" charset="-120"/>`;
    }
  }
  ...
  if (opts.hyperlink) {
    if (typeof opts.hyperlink !== "object") {
      ...
      runProps +=
        `<a:hlinkClick r:id="rId${opts.hyperlink._rId}" invalidUrl="" action="" tgtFrame="" tooltip="${
      ...
      runProps +=
        `<a:hlinkClick r:id="rId${opts.hyperlink._rId}" action="ppaction://hlinksldjump" tooltip="${
          opts.hyperlink.tooltip
            ? encodeXmlEntities(opts.hyperlink.tooltip)
            : ""
        }"${opts.color ? ">" : "/>"}`;
...
function genXmlTextRun(textObj: TextProps): string {
  ...
  return textObj.text
    ? `<a:r>${genXmlTextRunProperties(textObj.options, false)}<a:t>${
...
function genXmlBodyProperties(slideObject: ISlideObject | TableCell): string {
  ...
  if (
    slideObject && slideObject._type === SLIDE_OBJECT_TYPES.text &&
...
export function genXmlTextBody(slideObj: ISlideObject | TableCell): string {
  ...
  if (
    opts && slideObj._type !== SLIDE_OBJECT_TYPES.tablecell &&
  ...
  // STEP 2: Add bodyProperties
  {
    ...
    if (opts.h === 0 && opts.line && opts.align) {
      ...
      strSlideXml += `<a:lstStyle>${
        genXmlParagraphProperties(slideObj, true)
      }</a:lstStyle>`;
  ...
  if (typeof slideObj.text === "string" || typeof slideObj.text === "number") {
    ...
    slideObj.text && !Array.isArray(slideObj.text) &&
  ...
  arrTextObjects.forEach((textObj, idx) => {
    ...
    if (arrTexts.length > 0 && (textObj.options.align || opts.align)) {
      ...
      arrTexts.length > 0 && textObj.options.bullet && arrTexts.length > 0
    ) {
      arrLines.push(arrTexts);
      arrTexts = [];
  ...
  arrLines.forEach((line) => {
    ...
    let paragraphPropXml = `<a:pPr ${
      line[0].options?.rtlMode ? ' rtl="1" ' : ""
    }`;
    ...
    line.forEach((textObj, idx) => {
      ...
      textObj.options.lineSpacing = textObj.options.lineSpacing ||
        opts.lineSpacing;
      textObj.options.lineSpacingMultiple =
        textObj.options.lineSpacingMultiple || opts.lineSpacingMultiple;
      textObj.options.indentLevel = textObj.options.indentLevel ||
        opts.indentLevel;
      textObj.options.paraSpaceBefore = textObj.options.paraSpaceBefore ||
        opts.paraSpaceBefore;
      textObj.options.paraSpaceAfter = textObj.options.paraSpaceAfter ||
        opts.paraSpaceAfter;
    ...
    if (
      ...
      if (opts.fontFace) {
        strSlideXml += `<a:endParaRPr lang="${opts.lang || "en-US"}"` +
...
export function makeXmlContTypes(
  ...
  slides.forEach((slide) => {
    (slide._relsMedia || []).forEach((rel) => {
      if (
        rel.type !== "image" && rel.type !== "online" && rel.type !== "chart" &&
        rel.extn !== "m4v" && !strXml.includes(rel.type)
      ) {
        strXml += '<Default Extension="' + rel.extn + '" ContentType="' +
          rel.type + '"/>';
      }
    });
  });
  ...
  slideLayouts.forEach((layout, idx) => {
    ...
    (layout._relsChart || []).forEach((rel) => {
      strXml += ' <Override PartName="' + rel.Target +
...
export function makeXmlSlide(slide: PresSlide): string {
  return (
    ...
    `<p:clrMapOvr><a:masterClrMapping/></p:clrMapOvr>${
      genXmlSlideTransition(slide.transition)
    }</p:sld>`
  );
}
...
export function makeXmlPresentation(pres: IPresentationProps): string {
  ...
  pres.slides.forEach(
    (
      ...
      `<p:sldId id="${slide._slideId}" r:id="rId${slide._rId}"/>`),
  );
  strXml += "</p:sldIdLst>";
  ...
  // STEP 4: Add sizes
  strXml +=
    `<p:sldSz cx="${pres.presLayout.width}" cy="${pres.presLayout.height}"/>`;
  strXml +=
  ...
  if (pres.sections && pres.sections.length > 0) {
    ...
    pres.sections.forEach((sect) => {
      strXml += `<p14:section name="${encodeXmlEntities(sect.title)}" id="{${
...
export function correctShadowOptions(shadowProps: ShadowProps): void {
  ...
  // OPT: `type`
  if (
    shadowProps.type !== "outer" && shadowProps.type !== "inner" &&
  ...
  if (shadowProps.angle) {
    // A: REALITY-CHECK
    if (
      isNaN(Number(shadowProps.angle)) || shadowProps.angle < 0 ||
  ...
  if (shadowProps.opacity) {
    // A: REALITY-CHECK
    if (
      isNaN(Number(shadowProps.opacity)) || shadowProps.opacity < 0 ||
