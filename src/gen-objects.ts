...
import { getSlidesForTableRows } from "./gen-tables.ts";
import {
...
export function addChartDefinition(
  ...
  // A: Core
  options._type = type;
  options.x = typeof options.x !== "undefined" && options.x != null &&
      !isNaN(Number(options.x))
    ? options.x
    : 1;
  options.y = typeof options.y !== "undefined" && options.y != null &&
  ...
  options.objectName = options.objectName
    ...
    : `Chart ${
      target._slideObjects.filter((obj) =>
        obj._type === SLIDE_OBJECT_TYPES.chart
      ).length
    }`;
  ...
  if (options.dataLabelPosition) {
    if (
      options._type === CHART_TYPE.AREA || options._type === CHART_TYPE.BAR3D ||
    ...
    if (
      ...
      options._type === CHART_TYPE.LINE || options._type === CHART_TYPE.SCATTER
    ) {
  ...
  options.catGridLine = options.catGridLine ||
  ...
  options.serGridLine = options.serGridLine ||
    (options._type === CHART_TYPE.SCATTER
      ? { color: "D9D9D9", size: 1 }
      : { style: "none" });
  ...
  // C: Options: plotArea
  options.showDataTable = options.showDataTable || !options.showDataTable
  ...
  options.showDataTableKeys =
    options.showDataTableKeys || !options.showDataTableKeys
      ? options.showDataTableKeys
      : true;
  options.showLabel = options.showLabel || !options.showLabel
    ? options.showLabel
    : false;
  options.showLegend = options.showLegend || !options.showLegend
    ? options.showLegend
    : false;
  options.showPercent = options.showPercent || !options.showPercent
    ? options.showPercent
    : true;
  options.showTitle = options.showTitle || !options.showTitle
    ? options.showTitle
    : false;
  options.showValue = options.showValue || !options.showValue
    ? options.showValue
    : false;
  options.showLeaderLines = options.showLeaderLines || !options.showLeaderLines
    ? options.showLeaderLines
    : false;
  options.catAxisLineShow = typeof options.catAxisLineShow !== "undefined"
    ? options.catAxisLineShow
    : true;
  options.valAxisLineShow = typeof options.valAxisLineShow !== "undefined"
    ? options.valAxisLineShow
    : true;
  options.serAxisLineShow = typeof options.serAxisLineShow !== "undefined"
  ...
  options.v3DRotY =
    !isNaN(options.v3DRotY) && options.v3DRotY >= 0 && options.v3DRotY <= 360
      ? options.v3DRotY
      : 30;
  options.v3DRAngAx = options.v3DRAngAx || !options.v3DRAngAx
    ? options.v3DRAngAx
    : true;
  options.v3DPerspective =
    !isNaN(options.v3DPerspective) && options.v3DPerspective >= 0 &&
  ...
  options.barGapWidthPct =
    !isNaN(options.barGapWidthPct) && options.barGapWidthPct >= 0 &&
      options.barGapWidthPct <= 1000
      ? options.barGapWidthPct
      : 150;
  options.barGapDepthPct =
    !isNaN(options.barGapDepthPct) && options.barGapDepthPct >= 0 &&
  ...
  options.border = options.border && typeof options.border === "object"
    ? options.border
    : null;
  ...
  if (options.dataBorder && options.dataBorder.color) {
    const isHexColor = typeof options.dataBorder.color === "string" &&
  ...
  options.dataLabelFormatCode = options.dataLabelFormatCode &&
  ...
  //
  options.lineSize = typeof options.lineSize === "number"
    ? options.lineSize
    : 2;
  options.valAxisMajorUnit = typeof options.valAxisMajorUnit === "number"
    ? options.valAxisMajorUnit
    : null;

  if (
    options._type === CHART_TYPE.AREA || options._type === CHART_TYPE.BAR ||
    options._type === CHART_TYPE.BAR3D || options._type === CHART_TYPE.LINE
  ) {
...
export function addImageDefinition(target: PresSlide, opt: ImageProps): void {
  ...
  const objectName = opt.objectName
    ...
    : `Image ${
      target._slideObjects.filter((obj) =>
        obj._type === SLIDE_OBJECT_TYPES.image
      ).length
    }`;
  ...
  if (!strImagePath && !strImageData) {
    ...
    strImageData && typeof strImageData === "string" &&
  ...
  if (
    strImageData && /image\/(\w+);/.exec(strImageData) &&
  ...
  if (strImgExtn === "svg") {
    throw new Error("SVG images are not supported in this Deno-only fork.");
  }
  ...
  const dupeItem =
    target._relsMedia.filter((item) =>
      item.path && item.path === strImagePath &&
      item.type === "image/" + strImgExtn && !item.isDuplicate
    )[0];
...
export function addMediaDefinition(target: PresSlide, opt: MediaProps): void {
  ...
  if (strType === "online") {
    ...
    const dupeItem = target._relsMedia.filter((item) =>
      item.path && item.path === strPath &&
      item.type === strType + "/" + strExtn && !item.isDuplicate
    )[0];
...
export function addShapeDefinition(
  ...
  options.objectName = options.objectName
    ...
    : `Shape ${
      target._slideObjects.filter((obj) =>
        obj._type === SLIDE_OBJECT_TYPES.text
      ).length
    }`;
...
export function addTableDefinition(
  ...
  opt.objectName = opt.objectName
    ...
    : `Table ${
      target._slideObjects.filter((obj) =>
        obj._type === SLIDE_OBJECT_TYPES.table
      ).length
    }`;
  ...
  // STEP 1: REALITY-CHECK
  {
    // A: check for empty
    if (
      tableRows === null || tableRows.length === 0 || !Array.isArray(tableRows)
    ) {
      throw new Error(
  ...
  opt.autoPageRepeatHeader = typeof opt.autoPageRepeatHeader === "boolean"
    ? opt.autoPageRepeatHeader
    : false;
  opt.autoPageHeaderRows = typeof opt.autoPageHeaderRows !== "undefined" &&
      !isNaN(Number(opt.autoPageHeaderRows))
    ? Number(opt.autoPageHeaderRows)
    : 1;
  opt.autoPageLineWeight = typeof opt.autoPageLineWeight !== "undefined" &&
  ...
  if (opt.colW) {
    ...
    if (typeof opt.colW === "string" || typeof opt.colW === "number") {
      ...
      opt.colW && Array.isArray(opt.colW) && opt.colW.length === 1 &&
      firstRowColCnt > 1
    ) {
      ...
      opt.colW && Array.isArray(opt.colW) && opt.colW.length !== firstRowColCnt
    ) {
  ...
  // Used internally by auto-paging so we can add a pre-split table chunk without recursing.
  if (opt && !opt.autoPage) {
    ...
    if (opt.autoPageRepeatHeader) {
      opt._arrObjTabHeadRows = arrRows.filter((_row, idx) =>
        idx < opt.autoPageHeaderRows
      );
    }
  ...
  function cleanOpts(itemOpts: ObjectOptions): TextPropsOptions {
    // STEP 1: Set some options
    {
      ...
      if (!itemOpts.placeholder) {
        itemOpts.color = itemOpts.color || newObject.options.color ||
          target.color || DEF_FONT_COLOR;
      }
      ...
      itemOpts._bodyProp.wrap = typeof itemOpts.wrap === "boolean"
        ? itemOpts.wrap
        : true;
      ...
      if (
        typeof itemOpts.underline === "boolean" && itemOpts.underline === true
...
export function addPlaceholdersToSlideLayouts(slide: PresSlide): void {
  ...
  (slide._slideLayout._slideObjects || []).forEach((slideLayoutObj) => {
    if (slideLayoutObj._type === SLIDE_OBJECT_TYPES.placeholder) {
      ...
      if (
        slide._slideObjects.filter((slideObj) =>
          slideObj.options &&
...
export function addBackgroundDefinition(
  ...
  if (props && (props.path || props.data)) {
    ...
    if (
      strImgExtn === "svg" ||
      props.data?.toLowerCase().includes("image/svg+xml")
    ) {
      throw new Error(
        "SVG background images are not supported in this Deno-only fork.",
      );
    }
  ...
  textObjs.forEach((text, idx: number) => {
    ...
    if (Array.isArray(text)) {
      ...
      createHyperlinkRels(
        target,
        text.text,
        options && options[idx] ? [options[idx]] : undefined,
      );
    } else if (
      text && typeof text === "object" && text.options &&
      text.options.hyperlink && !text.options.hyperlink._rId
    ) {
      ...
      if (
        target._rels.filter((rel) => rel.rId === text.options.hyperlink._rId)
          .length === 0
      ) {
        target._rels.push({
          type: SLIDE_OBJECT_TYPES.hyperlink,
