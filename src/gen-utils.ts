...
export function encodeXmlEntities(xml: string): string {
  ...
  return xml.toString().replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(
    />/g,
    "&gt;",
  ).replace(/"/g, "&quot;").replace(/'/g, "&apos;");
}
...
export function createColorElement(
  ...
  const colorAttr = 'val="' +
    (REGEX_HEX_COLOR.test(colorVal) ? colorVal.toUpperCase() : colorVal) + '"';
...
export function createGlowElement(
  options: TextGlowProps,
  defaults: TextGlowProps,
): string {
  let strXml = "";
...
export function genXmlColorSelection(
  ...
  if (props) {
    if (typeof props === "string") colorVal = props;
      ...
      if (props.alpha) {
        internalElements += `<a:alpha val="${
          Math.round((100 - props.alpha) * 1000)
        }"/>`; // DEPRECATED: @deprecated v3.3.0
      }
      if (props.transparency) {
        internalElements += `<a:alpha val="${
          Math.round((100 - props.transparency) * 1000)
        }"/>`;
      }
    }
 ...
 */
export function correctShadowOptions(
